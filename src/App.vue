  ...
  :style="xs?{height: '100%',width: '100%',top: '0',left:'0'}:(sm?{height: '98%',width: '98%',top: '1%',left:' 1%'}:{height: '96.6%',width: '99%',top: '1.7%',left:' 0.5%'})">
    ...
    <div class="global-player-wrapper" @mouseenter="!xs && musicplayershow(1)" @mouseleave="!xs && musicplayershow(0)" @click="xs && showMusicPlaylist()">
      <v-card class="musicplayer" variant="tonal">
        ...
        <div style="display:flex;align-items:center;gap:8px;">
          <div style="display:flex;flex-direction:column;justify-content:center;">
            <span ref="audiotitle" class="musicplayer-text" style="font-weight: bolder;">{{ musicinfo?.[playlistIndex]?.title }}</span>
            <span ref="audioauthor" class="musicplayer-text" style="opacity:0.85;font-size:12px;">{{ musicinfo?.[playlistIndex]?.author }}</span>
          </div>
          <audio v-show="false" ref="audioPlayer" :src="musicinfo?.[playlistIndex]?.url" @waiting="onWaiting" @canplay="onCanPlay" :preload="configdata.musicConfig?.preload ? 'auto' : 'metadata'" @play="onPlay" @playing="onPlaying" @pause="onPause"></audio>
      ...
      <transition name="slide-down">
        <v-card v-show="ismusicplayer" class="ma-2 pa-2 music-playlist-panel" variant="tonal" @click.stop>
          <template v-slot:title>
            <span>播放列表</span>
          </template>
          <v-list style="max-height: 220px; overflow-y: auto; background: transparent; width: 320px;">
            <v-list-item v-for="(song, index) in musicinfo" :key="index" @click="playlistIndex = index; updateAudio();">
              <v-list-item-content class="d-flex justify-space-between">
                <v-list-item-title style="color: var(--leleo-vcard-color);">{{ index + 1 }}. {{ song.title }}</v-list-item-title>
            ...
            <v-col cols="12" md="4" lg="3" class="leleo-left" align="center">
              <div :style="xs||sm?{'font-size':'2.3rem'}:{'display':'none'}" class="leleo-left-welcome">{{ configdata.welcometitle }}</div>  
                <v-avatar class="leleo-left-avatar" :size="xs||sm?120:140" :style="xs||sm?{'margin-top': '0'}:{'margin-top': '2rem'}">
                  ...
                  <!-- 播放器已移至页面左上角固定显示（见页面顶部的 global-musicplayer） -->
                </v-avatar>
                ...
                <!-- 下滑播放列表面板，hover 全局播放器或头像时显示 -->
                <transition name="slide-down">
                  <v-card v-show="ismusicplayer" class="ma-2 pa-2 music-playlist-panel" variant="tonal" :max-width="xs?270:300" @click.stop>
                    <template v-slot:title>
                      <span>播放列表</span>
                    </template>
                    <v-list style="max-height: 220px; overflow-y: auto; background: transparent;">
                      <v-list-item v-for="(song, index) in musicinfo" :key="index" @click="playlistIndex = index; updateAudio();">
                        <v-list-item-content class="d-flex justify-space-between">
                          <v-list-item-title style="color: var(--leleo-vcard-color);">{{ index + 1 }}. {{ song.title }}</v-list-item-title>
                          <span style="font-size: 12px; color: var(--leleo-vcard-color);">{{ song.author }}</span>
