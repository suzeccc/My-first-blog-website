...
export default {
    ...
    watch: {
        tab(val) {
            ...
            if(val == 'tab-1'){
                this.wallpaperPIC = this.type === 'mobile' 
                    ? (this.configdata.wallpaper.picMobile || this.configdata.wallpaper.pic || [])
                    : (this.configdata.wallpaper.pic || []);
            }else{
                this.wallpaperVD = this.type === 'mobile' 
                    ? (this.configdata.wallpaper.videoMobile || this.configdata.wallpaper.video || [])
        ...
        switchType(type,tabtype){
            if(tabtype == 'static'){
                if(type == 'mobile'){
                    ...
                    if (!this.configdata.wallpaper.picMobile || this.configdata.wallpaper.picMobile.length === 0) {
                        this.wallpaperPIC = this.configdata.wallpaper.pic || [];
                        console.log('移动端静态壁纸为空，使用PC端壁纸作为后备');
                    }
                }else if(type == 'pc'){
                    this.type='pc';
                    this.itemsPerPage = 6;
                ...
                if(type == 'mobile'){
                    ...
                    if (!this.configdata.wallpaper.videoMobile || this.configdata.wallpaper.videoMobile.length === 0) {
                        this.wallpaperVD = this.configdata.wallpaper.video || [];
                        console.log('移动端动态壁纸为空，使用PC端壁纸作为后备');
                    }
                }else if(type == 'pc'){
                    this.type='pc';
                    this.itemsPerPage = 6;
